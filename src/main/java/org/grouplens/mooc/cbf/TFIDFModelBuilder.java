...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
    ...
    public TFIDFModel get() {
        ...
        for (long item: items) {
            ...
            List<Long> itemTagIds = new ArrayList<Long>();
            double init = 1;
            for(String tag: itemTags){
                itemTagIds.add(tagIds.get(tag));

            }

            for(Long tagId: itemTagIds){
                if(work.containsKey(tagId)) {
                   Double freq = work.get(tagId);
                   work.set(tagId, freq + 1);
                   }
                else{
                    work.set(tagId , init);
                }
            ...
            for(Long uTag: work.keySet()){
                double prev = docFreq.get(uTag);
                docFreq.set(uTag , prev + 1);
            }
        ...
        double totalNumOfItems = items.size();
        ...
        for (VectorEntry e: docFreq.fast()) {
            double numOfItemsInWhichTermOccurs = e.getValue();
            double idfTerm = Math.log(totalNumOfItems/numOfItemsInWhichTermOccurs);
            docFreq.set(e, idfTerm);
        }
        ...
        for (Map.Entry<Long,MutableSparseVector> entry: itemVectors.entrySet()) {
            ...
            LongSet setOfTagIdsAppliedToThisMovie =  tv.keySet();


            // TODO Convert this vector to a TF-IDF vector
            for(Long tagId: setOfTagIdsAppliedToThisMovie){
                double idfTerm = tv.get(tagId)*docFreq.get(tagId);
                tv.set(tagId,idfTerm);
