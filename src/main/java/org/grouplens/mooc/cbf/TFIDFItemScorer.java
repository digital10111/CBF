...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector output) {
        ...
        for (VectorEntry e: output.fast(VectorEntry.State.EITHER)) {
            ...
            Collection<Double> userScoreForAllTags = new ArrayList<Double>();
            userScoreForAllTags = userVector.values();
            Double sqrtOfTagScore = 0.0;
            for(Double tScore: userScoreForAllTags){
                sqrtOfTagScore = Math.pow(tScore,2) + sqrtOfTagScore;
            }
            sqrtOfTagScore = Math.sqrt(sqrtOfTagScore);
            Collection<Double> itemScoreForAllTags = new ArrayList<Double>();
            itemScoreForAllTags = iv.values();
            Double sqrtOfItemTagScore = 0.0;

            for(Double tIScore: itemScoreForAllTags){
                sqrtOfItemTagScore = Math.pow(tIScore,2) + sqrtOfItemTagScore;
            }
            sqrtOfItemTagScore = Math.sqrt(sqrtOfItemTagScore);
            Double denom = sqrtOfItemTagScore * sqrtOfTagScore;
            double numer = userVector.dot(iv);
            double cosine = numer/denom;
            output.set(e,cosine);
        }
    }
    ...
    private SparseVector makeUserVector(long user) {
        ...
        for (Rating r: userRatings) {
            ...
            if(p != null)   {
                meanValue = p.getValue() + meanValue;
            }
        }
        meanValue = meanValue/(userRatings.size());
        ...
        for (Rating r: userRatings) {
            ...
            if (p != null) {
                ...
                MutableSparseVector aux= model.getItemVector(p.getItemId()).mutableCopy();
                aux.multiply(p.getValue() - meanValue);
                profile.add(aux);
            }
        }
